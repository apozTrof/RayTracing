...
MatteT::MatteT()
:   Material(),
    m_textureDiffuseBRDF(new TextureDiffuse)
{}

MatteT::MatteT(const MatteT& a_mt)
:   Material(a_mt)
{
    if (a_mt.m_textureDiffuseBRDF)
    {
        m_textureDiffuseBRDF = a_mt.m_textureDiffuseBRDF;
    }
    else
    {
        m_textureDiffuseBRDF = std::make_shared<TextureDiffuse>(TextureDiffuse());
    }
}
...
RGBColor MatteT::shade(Intersection& a_inter) const
{
    ...
    for (int i = 0; i < nbLights; ++i)
    {
        ...
        {
            L += m_textureDiffuseBRDF->f(a_inter, wi, wo) * a_inter.m_pWorld->m_lights[i]->getRadiance(a_inter) * a_inter.m_pWorld->m_lights[i]->g(a_inter) * cosOmegaI / a_inter.m_pWorld->m_lights[i]->pdf(a_inter);
        }
    }
